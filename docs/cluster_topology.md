...
The data-source-based persistence uses list-index matching (`replication_specs[gi]`) to correlate each shard between the resource and the data source. This limitation does not apply to dedicated clusters with exactly one `replication_specs` entry, including replica set clusters and sharded or geosharded clusters with one shard. For current multi-shard topology-change guidance, see [Multi-shard clusters and topology changes](https://registry.terraform.io/providers/mongodb/mongodbatlas/latest/docs/resources/advanced_cluster#multi-shard-clusters-and-topology-changes) in the provider documentation, and contact [MongoDB Support](https://www.mongodb.com/docs/atlas/support/#request-support) for significant production changes.
