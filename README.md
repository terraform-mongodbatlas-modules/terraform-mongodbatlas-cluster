...
- Prefer `shard_name` for shard grouping. It must match `^[a-z0-9]{0,23}$` (lowercase alphanumeric; length 0 to 23). Do not set both `shard_name` and `shard_number` on the same region. Do not set either when `shard_count` is set.
