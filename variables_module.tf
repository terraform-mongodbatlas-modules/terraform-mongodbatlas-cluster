...
EOT
  ...
  validation {
    condition = alltrue([
      for r in var.regions :
      r.shard_name == null || can(regex("^[a-z][a-z0-9]{0,23}$", r.shard_name))
    ])
    error_message = "Each `regions[*].shard_name` must match ^[a-z][a-z0-9]{0,23}$ (starts with a lowercase letter; lowercase alphanumeric; length 1 to 24)."
  }
  validation {
    condition = alltrue([
      for r in var.regions :
