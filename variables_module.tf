...
EOT
  ...
  validation {
    condition = alltrue([
      for r in var.regions :
      r.shard_name == null || can(regex("^[a-z0-9]{0,23}$", r.shard_name))
    ])
    error_message = "Each `regions[*].shard_name` must match ^[a-z0-9]{0,23}$ (lowercase alphanumeric; length 0 to 23)."
  }
  validation {
    condition = alltrue([
      for r in var.regions :
