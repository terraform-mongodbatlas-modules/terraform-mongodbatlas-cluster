...
def generate_terraform_variable(
    provider: str,
    mappings: dict[str, dict],
    output_dir: Path,
    include_invalid: bool = False,
) -> Path:
    """Generate a variables_generated.tf file with an atlas_to_{provider}_region variable."""
    provider_lower = provider.lower()

    lines = [f"# variables_generated.tf -- generated by: just extract-regions {provider_lower}"]
    lines.append(f'variable "atlas_to_{provider_lower}_region" {{')
    ...
    lines.append("    Override to restrict allowed regions or add custom mappings.")
    lines.append("  EOT")
    lines.append("}")
    lines.append("")

    output_file = output_dir / "variables_generated.tf"
    with open(output_file, "w") as f:
        f.write("\n".join(lines))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-format",
        choices=["variable", "locals"],
        default="locals",
        help="'variable' => variables_generated.tf, 'locals' => regions_{provider}.tf",
    )
...
def main():
    ...
    if provider:
        ...
        generate_fn = (
            ...
            if args.output_format == "variable"
            else generate_terraform_locals
        )
        output_file = generate_fn(
            provider,
